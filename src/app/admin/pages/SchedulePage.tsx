...
type DeleteTarget = { entity: ScheduleEntity; id: string; title: string; description: string };
...
export function SchedulePage({ customers, onLoggedOut }: Props) {
  ...
  async function deleteEntity() {
    ...
    const response = await adminApi.deleteScheduleEntity(deleteTarget.entity, deleteTarget.id);
    setBundle(normalizeBundle(response));
...
function SettingsNumber({ label, value, unit, onChange }: { label: string; value: number; unit: string; onChange: (value: number) => void }) { return <label className="flex items-center justify-between gap-4"><span className="text-[11px] font-medium text-[#3f5d71]">{label}</span><span className="relative w-[132px]"><input type="number" min="0" className="admin-field w-full pr-14 text-right" value={value} onChange={(e) => onChange(Math.max(0, Number(e.target.value)))} /><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[9px] text-[#7c909d]">{unit}</span></span></label>; }
