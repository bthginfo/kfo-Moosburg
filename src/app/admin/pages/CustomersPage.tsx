...
function EmailConsentIndicators({ customer, compact = false }: { customer: Customer; compact?: boolean }) { const emailAvailable = Boolean(customer.email); return <div className={`flex ${compact ? "flex-row flex-wrap gap-3" : "flex-col gap-1.5"}`}><span className={`inline-flex items-center gap-1.5 text-[12px] font-medium ${customer.reminderConsent && emailAvailable ? "text-[#347357]" : "text-[#8a7770]"}`}>{customer.reminderConsent && emailAvailable ? <CheckCircle2 className="h-3.5 w-3.5" /> : <BellOff className="h-3.5 w-3.5" />}Termin</span><span className={`inline-flex items-center gap-1.5 text-[12px] font-medium ${customer.invoiceEmailConsent && emailAvailable ? "text-[#347357]" : "text-[#8a7770]"}`}><ReceiptText className="h-3.5 w-3.5" />Rechnung {customer.invoiceEmailConsent && emailAvailable ? "erlaubt" : "nicht freigegeben"}</span></div>; }
